...
class PenaltyMethod : public Function
{
public:
   Test test;
   int funct_n = 0;
   ...
   PenaltyMethod(const Test& t_test, int t_funct_n) :
      test(t_test), funct_n(t_funct_n) { }

   PenaltyMethod(const Test& t_test) : test(t_test) { }
   ...
   double G(const vector<double>& x) const
   {
      ...
      else
      {
         switch(funct_n)
         {
            ...
            case 2: return pow(0.5 * (test.g(x) + abs(test.g(x))), 8);
